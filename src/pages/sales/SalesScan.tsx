...
export default function SalesScan() {
  const navigate = useNavigate();
  const regionId = "sales-qr-region";
    ...
    try {
      ...
      await scanner.start(
        ...
        (decoded) => {
          ...
          if (!slug) return;
          stop().then(() => navigate(`/sales/items/${slug}`));
        },
        () => {},
      );
      setScanning(true);
  ...
  const goManual = () => {
    ...
    if (slug) navigate(`/sales/items/${slug}`);
  };
    ...
    <div className="min-h-screen bg-background px-6 py-10">
      <div className="max-w-lg mx-auto">
        <div className="flex items-center gap-2 mb-1">
          ...
          <h1 className="text-xl font-medium">Scan QR Produk</h1>
        </div>
        <p className="text-sm text-muted-foreground mb-6">
          Scan QR furniture untuk melihat rincian: stok, dimensi, material, berat, harga, dan item
          serupa.
        </p>
        ...
        <div className="mt-6">
          ...
          <div className="flex gap-2 mt-2">
            <input
              ...
              placeholder="mis. sofa-calma"
              className="flex-1 border border-border rounded px-3 py-2 text-sm bg-background"
            />
            <button
              onClick={goManual}
              className="px-4 py-2 border border-border rounded text-sm inline-flex items-center gap-2"
            >
