              ...
              <div className="mb-6">
                ...
                <ul className="space-y-1 text-sm">
                  {selected.items.map((it) => (
                    <li key={it.id} className="flex justify-between border-b border-border/40 py-1.5">
                      <span>{it.entity?.name ?? "—"}</span>
                      <span className="text-xs uppercase text-muted-foreground">{it.type}</span>
                    </li>
                  ))}
                </ul>
              </div>
