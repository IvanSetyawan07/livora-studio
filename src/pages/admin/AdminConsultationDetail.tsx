...
function DocumentsSection({ consultation, files }: { consultation: Consultation; files: ConsultationStageFile[] }) {
  ...
  const workflow = files.map((item) => ({ key: `file-${item.id}`, label: String(item.kind).replace(/_/g, " "), path: item.file_path, meta: new Date(item.created_at).toLocaleString("id-ID") }));
